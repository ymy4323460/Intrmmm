...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        # input_shape [2*28*28] num_domains*picture_size
        # hparams {'data_augmentation': True, 'resnet18': False, 'resnet_dropout': 0.0, 'class_balanced': False, 'nonlinear_classifier': False, 'lr': 0.001, 'weight_decay': 0.0, 'batch_size': 64}
    ...
    def update(self, minibatches, unlabeled=None):
        # minibatches len=2
        all_x = torch.cat([x for x, y, _ in minibatches])  # [128*2*28*28]
        all_y = torch.cat([y for x, y, _ in minibatches])  # [128]
        all_y_hat = self.predict(all_x)  # [128*2]
        loss = F.cross_entropy(all_y_hat, all_y)
...
#             var[i] = torch.ones(dim)*1
    ...
    def mmd(self, x, y, kernel_type = "gaussian"):
...
#     def conditional_independent(self, z, env_i):
#
#         descrepancy = 0
...
#                 index_j = env_i == j
#                 z_j = z[index_j]
#                 descrepancy += self.mmd(z_i, z_j)
#         return descrepancy

    def all_loss(self, v, z, y_pred, int_y_pred, intervention, z_c, y, turn='min'):
...
#             var[i] = torch.ones(dim)*1
    ...
    def intervention_loss(self, intervention):
#         print(torch.norm(torch.pow(intervention, 2)-self.hparams['bias']))
        return torch.norm(torch.pow(intervention, 2)-self.hparams['bias'])
    ...
    def kl_loss(self, m, v, y):
        if self.hparams['prior_type'] == 'conditional':
            pm, pv = self.condition_prior([30., 2.], y, m.size()[1])
        else:
            pm, pv = torch.zeros_like(m), torch.ones_like(m)
        # print(m)
        return self.kl_normal(m, pv * 0.0001, pm, pv * 0.0001)
...
#         print(z_c.size())
        ...
        if turn == 'min':
            ...
            return -(nll+self.hparams['int_lambda']*int_nll+ self.hparams['target_lambda']*targets_loss) + self.hparams['kl_lambda']*kl
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for i, (x, y, env_i) in enumerate(minibatches):
            if i % 100 == 0 and i > 0:
                self.max_optimizer.zero_grad()
                loss, irm_panelty = self.all_loss(x, y, 'max')
                L = loss.mean()
                L.backward()
                self.max_optimizer.step()
...
#         print(z_c.size())
        ...
        all = nll + self.hparams['int_lambda']*int_nll + self.hparams['int_reg']*inter_norm + self.hparams['target_lambda']*targets_loss
        if turn == 'min':
            return all  + self._irm_penalty(y_pred, y), self._irm_penalty(y_pred, y) #+ self.hparams['kl_lambda']*kl
        else:
            return -(nll+self.hparams['int_lambda']*int_nll) #+ self.hparams['kl_lambda']*kl
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        penalty = 0.
        for i, (x, y, env_i) in enumerate(minibatches):
            nll_0, penalty_0 = self.all_loss(x, y, env_i, 'min')
            nll += nll_0.mean()
            penalty += penalty_0.mean()


        nll /= len(minibatches)
        penalty /= len(minibatches)
        L = nll + (penalty_weight * penalty)
