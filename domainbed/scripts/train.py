...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', '-d',type=str, default="RotatedMNIST")
    parser.add_argument('--algorithm', '-a', type=str, default="ERM")
...
#     hparams['bias'] = args.hyper_bias
    ...
    for step in range(start_step, n_steps):
        ...
        minibatches_device = [(x.to(device), y.to(device), env_id.to(device))
            for x,y,env_id in next(train_minibatches_iterator)]

        if args.task == "domain_adaptation":
            uda_device = [(x.to(device), env_id.to(device))
                for x, _, env_id in next(uda_minibatches_iterator)]
        else:
            uda_device = None
